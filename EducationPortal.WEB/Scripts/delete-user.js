(function (window) {
    let timerId = null;
    let btns = document.getElementsByClassName('delete-user-btn');

    for (let i = 0; i < btns.length; i++) {
        btns[i].onclick = function () {
            let postSend = new XMLHttpRequest();
            postSend.onreadystatechange = onReadyStateChange;
            postSend.open('POST', location.protocol + "//" + location.host + '/Account/DeleteUser', true);
        ...
        if (this.readyState === 4 && this.status === 200) {
            ...
            try {
                ...
                if (obj.Message == 'OK') {
                    ...
                    message.textContent = 'Пользователь удалён';
                }
                else if (obj.Message == 'UserIsAbsent') {
                    let removedElem = document.getElementById(obj.Index);
                    removedElem.remove();
                    ...
                    message.textContent = 'Пользователь уже был удалён';
                }

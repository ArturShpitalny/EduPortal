(function (window) {
    let timerId = null;
    let btns = document.getElementsByClassName('delete-skill-btn');

    for (let i = 0; i < btns.length; i++) {
        btns[i].onclick = function () {
            let postSend = new XMLHttpRequest();
            postSend.onreadystatechange = onReadyStateChange;
            postSend.open('POST', location.protocol + "//" + location.host + '/Skill/DeleteSkill', true);
        ...
        if (this.readyState === 4 && this.status === 200) {
            ...
            try {
                ...
                if (obj.Message == 'OK') {
                    ...
                    message.textContent = 'Умение удалено';
                }
                else if (obj.Message == 'SkillIsStillInUse') {
                    message.textContent = 'Умение нельзя удалить, т.к. оно используется';
                }
                else if (obj.Message == 'SkillIsAbsent') {
                    let removedElem = document.getElementById(obj.Index);
                    removedElem.remove();
                    ...
                    message.textContent = 'Умение уже было удалено';
                }

(function (window) {
    let timerId = null;
    let btns = document.getElementsByClassName('delete-material-btn');

    for (let i = 0; i < btns.length; i++) {
        btns[i].onclick = function () {
            let postSend = new XMLHttpRequest();
            postSend.onreadystatechange = onReadyStateChange;
            postSend.open('POST', location.protocol + "//" + location.host + '/Material/DeleteMaterial', true);
    ...
    function onReadyStateChange() {
        if (this.readyState === 4 && this.status === 200) {
            ...
            try {
                ...
                else if (obj.Message == 'MaterialIsAbsent') {
                    let removedElem = document.getElementById(obj.Index);
                    removedElem.remove();
